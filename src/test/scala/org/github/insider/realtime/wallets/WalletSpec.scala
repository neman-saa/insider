...
object WalletSpec extends SimpleIOSuite {
  ...
  private def oldBuyTimes(tokens: String*): Map[String, Instant] =
    tokens.map(_ -> Instant.now().minusSeconds(3600)).toMap
  ...
  test("sells redundant and replaced positions before buying better top market") {
    ...
    for {
      ...
      List("b", "c"),
      operations.collect { case Operation.Sell(tokenId, _, _) => tokenId }.take(2)
    ) and
